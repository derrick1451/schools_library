...
require './book'
require './classroom'
require_relative 'io'  # Include the IO module
...
class App
  ...
  def load_data_from_files
    @books = IO.load_data_from_file('books.json')
    @people = IO.load_data_from_file('people.json')
    @rentals = IO.load_data_from_file('rentals.json')
  end

  # ... Existing methods ...

   # Save books, people, and rentals to JSON files using the IO module
   def save_data_to_files
    save_data_to_file(@books, 'books.json')
    save_data_to_file(@people.map(&:to_hash), 'people.json')
    save_data_to_file(@rentals, 'rentals.json')
   end
  
  # Load data from JSON files into books, people, and rentals
  def load_data_from_files
    @books = DataIO.load_data_from_file('books.json')
    @people = DataIO.load_data_from_file('people.json')
    @rentals = DataIO.load_data_from_file('rentals.json')
  end
